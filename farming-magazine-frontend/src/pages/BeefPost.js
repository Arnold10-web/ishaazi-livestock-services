  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        document.title = `${response.data.data.title} | Beef Articles`;
        
        const recentResponse = await axios.get(`${API_BASE_URL}/api/content/beefs?limit=3&exclude=${id}&sort=-createdAt`);
