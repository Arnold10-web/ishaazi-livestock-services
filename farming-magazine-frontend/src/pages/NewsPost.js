...
const NewsPost = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setNews(newsData);
        document.title = `${newsData.title} | Latest News`;
        
        const recentResponse = await axios.get(`${API_BASE_URL}/api/content/news?limit=4&exclude=${id}&sort=-createdAt`);
        setRecentNews(recentResponse.data.data.news);
  ...
  useEffect(() => {
    ...
    setNews(prev => ({ ...prev, content: doc.body.innerHTML }));
  }, [news]);
  ...
  const handleShare = (platform = null) => {
    ...
    const shareText = news.excerpt || news.title;
  ...
  const handleRecentNewsClick = (articleId) => {
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900">
        <div className="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-md max-w-md w-full text-center">
          <div className="flex items-center justify-center mb-4">
            <AlertOctagon className="h-16 w-16 text-gray-400" />
          </div>
      ...
      <div className="container mx-auto px-4 py-12 max-w-7xl">
        <div className="grid lg:grid-cols-3 gap-8">
            ...
            <article className="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden" ref={articleRef}>
              <div className="p-6 md:p-8">
                <header className="mb-8">
                  {news.category && (
                    <span className="inline-block bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 px-3 py-1 rounded-full text-sm font-medium mb-4">
                      {news.category}
                    </span>
                  )}
                  ...
                  <div className="flex flex-wrap items-center gap-4 text-gray-600 dark:text-gray-400 mb-6 text-sm">
                    ...
                    {news.author && (
                      <div className="flex items-center">
                        <UserCircle2 className="mr-2 h-4 w-4 text-blue-500" />
                ...
                <div 
                  className="prose prose-lg max-w-none dark:prose-invert 
                    prose-img:rounded-lg prose-img:shadow-md prose-a:text-blue-500 dark:prose-a:text-blue-400
                ...
                {news.tags && news.tags.length > 0 && (
                  <footer className="mt-12 pt-6 border-t border-gray-200 dark:border-gray-700">
                    <div className="flex flex-wrap gap-2 items-center">
                      <Tags className="h-5 w-5 text-gray-500 dark:text-gray-400 mr-2" />
          ...
          <aside className="space-y-6">
                  ...
                  {headings.map((heading) => (
                    <button
                      ...
                      className={`block text-left w-full px-2 py-1 rounded text-sm transition-colors
                        ...
                        hover:text-blue-500 dark:hover:text-blue-400 hover:bg-gray-100 dark:hover:bg-gray-700`}
                    >
                      {heading.text}
                    </button>
                  ))}
            ...
            {news.author && (
              <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm">
                <div className="flex items-center mb-4">
                  <UserCircle2 className="h-12 w-12 text-blue-500 dark:text-blue-400 mr-4" />
                  <div>
                    <h3 className="text-lg font-semibold text-gray-800 dark:text-white">{news.author}</h3>
                    <p className="text-sm text-gray-500 dark:text-gray-400">Staff Writer</p>
                  </div>
                </div>
                <p className="text-sm text-gray-600 dark:text-gray-300">
                  {news.authorBio || "Experienced journalist covering the latest news and insights."}
                </p>
              </div>
            )}
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm">
              <div className="flex items-center mb-6">
                <Newspaper className="h-5 w-5 text-blue-500 dark:text-blue-400 mr-3" />
                <h2 className="text-lg font-semibold text-gray-800 dark:text-white">Recent News</h2>
              </div>
              <div className="space-y-4">
                {recentNews.length > 0 ? (
                  recentNews.map(article => (
                    <div
                      key={article._id}
                      onClick={() => handleRecentNewsClick(article._id)}
                    ...
                    >
                      <div className="flex gap-4 items-start">
                        ...
                        <div>
                          <h3 className="text-sm font-medium text-gray-800 dark:text-white group-hover:text-blue-500 dark:group-hover:text-blue-400 transition-colors line-clamp-2">
                            {article.title}
                          </h3>
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400">No recent news found</p>
                )}
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm">
              ...
              <div className="space-y-4 text-sm">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-medium text-gray-800 dark:text-white">{estimateReadTime(news.content)}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-600 dark:text-gray-400">Published</span>
                  <span className="font-medium text-gray-800 dark:text-white">{formatDate(news.createdAt)}</span>
                </div>
                {news.updatedAt && (
                  <div className="flex justify-between items-center">
                    <span className="text-gray-600 dark:text-gray-400">Last Updated</span>
                    <span className="font-medium text-gray-800 dark:text-white">{formatDate(news.updatedAt)}</span>
                  </div>
                )}
              </div>
            </div>
