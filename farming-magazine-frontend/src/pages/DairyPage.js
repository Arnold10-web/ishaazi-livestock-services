...
const DairyPage = () => {
  ...
  const filteredAndSortedDairies = dairies
    .filter(dairy =>
  ...
  const indexOfLastDairy = currentPage * dairiesPerPage;
  const indexOfFirstDairy = indexOfLastDairy - dairiesPerPage;
  const currentDairies = filteredAndSortedDairies.slice(indexOfFirstDairy, indexOfLastDairy);
  const totalPages = Math.ceil(filteredAndSortedDairies.length / dairiesPerPage);

  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900">
        <Loader2 className="h-12 w-12 animate-spin text-blue-600" />
        <p className="mt-4 text-gray-700 dark:text-gray-300 font-semibold">Loading dairy content...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <header className="bg-blue-600 py-12 text-center text-white">
        <h1 className="text-4xl font-bold">Dairy Management</h1>
        <p className="mt-2 text-blue-100">Expert information on dairy farming and management techniques</p>
      </header>
      ...
      <main className="container mx-auto px-4 py-10">
        ...
        <AnimatePresence mode="wait">
          <DairyList dairies={currentDairies} apiBaseUrl={API_BASE_URL} isLoading={loading} />
        </AnimatePresence>
