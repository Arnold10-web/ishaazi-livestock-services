...
const DairyPost = () => {
  ...
  const handleShare = (platform = null) => {
    ...
    const shareTitle = article?.title || 'Dairy Article';
    const shareText = article?.description || article?.title || 'Check out this dairy article';
  ...
  if (error) return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900">
      <div className="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-md max-w-md w-full text-center">
        ...
        <Link to="/dairy" className="text-blue-600 hover:underline">Return to Dairy Articles</Link>
      </div>
    </div>
  );
    ...
    <div className="bg-gray-50 dark:bg-gray-900">
      ...
      <div className="container mx-auto px-4 py-12 max-w-6xl grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8" ref={articleRef}>
          ...
          >
            <ArrowLeft className="h-4 w-4 mr-2" /> Back to Dairy Articles
          </button>
          ...
          <article className="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow">
            <div className="space-y-6">
              ...
              <div className="flex flex-wrap gap-4 text-gray-600 dark:text-gray-400 text-sm">
                ...
                {article.category && (
                  <span className="flex items-center gap-1">
                    <Milk className="h-4 w-4" /> {article.category}
                  </span>
                )}
              </div>
              ...
              <div className="prose dark:prose-invert max-w-none prose-blue" dangerouslySetInnerHTML={{ __html: article.content }} />
        ...
        <aside className="space-y-6 sticky top-20 self-start">
          ...
          <RecentPosts posts={recentArticles} themeColor="#3b82f6" />
        </aside>
      </div>
    </div>
  );
