...
const ContentStats = ({ activeTab, content, darkMode }) => {
  ...
  const prepareChartData = () => {
    ...
    // Engagement data (if available) - Comments system removed
    const engagementData = [];
    if (content.some(item => item.views || item.likes)) {
      engagementData.push(
...
const ContentManagement = ({ activeTab, darkMode, actionToTrigger, onActionHandled }) => {
  ...
  // Handle action triggers from quick actions
  useEffect(() => {
    if (actionToTrigger === 'create' && activeTab !== 'subscribers' && activeTab !== 'registrations') {
      setFormVisible(true);
      if (onActionHandled) {
        onActionHandled();
      }
    }
  }, [actionToTrigger, activeTab, onActionHandled]);
  ...
  // Comment system has been removed for statistics accuracy
  ...
  const renderContent = () => {
    ...
    return (
      <div>
        ...
        {/* Send Push Notification Button and Form */}
