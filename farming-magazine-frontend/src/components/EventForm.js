...
const EventForm = ({ refreshEvents, editingEvent, setEditingEvent }) => {
  ...
  const storageKey = `event_form_${editingEvent?._id || 'new'}`;
  ...
  const {
    ...
    onSaveSuccess: () => console.log('Event auto-saved to localStorage'),
    onSaveError: (error) => console.error('Event auto-save failed:', error)
  });
