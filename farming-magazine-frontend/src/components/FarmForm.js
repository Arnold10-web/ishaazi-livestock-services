...
const FarmForm = ({ refreshFarms, editingFarm, setEditingFarm }) => {
  ...
  const storageKey = `farm_form_${editingFarm?._id || 'new'}`;
  ...
  const {
    ...
    onSaveSuccess: () => console.log('Farm listing auto-saved to localStorage'),
    onSaveError: (error) => console.error('Farm listing auto-save failed:', error)
  });
