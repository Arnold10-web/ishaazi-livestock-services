...
const BasicForm = ({ refreshBasics, editingBasic, setEditingBasic }) => {
  ...
  const storageKey = `basic_form_${editingBasic?._id || 'new'}`;
  ...
  const {
    ...
    onSaveSuccess: () => console.log('Basic content auto-saved to localStorage'),
    onSaveError: (error) => console.error('Basic content auto-save failed:', error)
  });
