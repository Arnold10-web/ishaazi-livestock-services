...
const BlogForm = ({ refreshBlogs, editingBlog, setEditingBlog }) => {
  ...
  const storageKey = editingBlog ? `blog-draft-${editingBlog._id}` : 'blog-draft-new';
  ...
  const {
    ...
    onSaveSuccess: (saveType) => {
      console.log(`✅ Blog auto-saved to ${saveType}`);
    },
    onSaveError: (error, saveType) => {
      console.error(`❌ Failed to auto-save blog to ${saveType}:`, error);
    }
  });
  ...
  useEffect(() => {
    if (editingBlog) {
      ...
      if (quillEditor) {
        quillEditor.root.innerHTML = blogContent;
      }
    } else {
      // Try to load draft when creating new blog
  ...
  }, [editingBlog, quillEditor, loadSavedData, clearSavedData]);
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-6">
        ...
        <div className="flex flex-wrap gap-3 pt-4">
          ...
          <button 
            type="button" 
            onClick={saveNow} 
            className="px-6 py-3 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:ring-offset-2 transition-all duration-200 flex items-center"
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
