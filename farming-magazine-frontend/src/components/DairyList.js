...
import { Edit2, Trash2, ArrowRight, Calendar, Clock, Milk, Tag, Heart, Eye } from 'lucide-react';
  ...
  if (dairies.length === 0 && !isLoading) {
    return (
      <div className="text-center p-12 bg-white dark:bg-gray-800 rounded-2xl shadow">
        <Milk className="w-12 h-12 mx-auto text-blue-600 dark:text-blue-400 mb-4" />
        <h3 className="text-xl font-bold text-gray-800 dark:text-white">No Dairy Information Found</h3>
        <p className="text-gray-600 dark:text-gray-400 mt-2">Check back later for dairy farming content.</p>
      </div>
    );
  }
  ...
  if (isLoading) {
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {[1, 2, 3].map(i => <DairySkeleton key={i} />)}
      </div>
    );
  }
        ...
        {dairies.map((dairy, index) => (
          ...
          >
              ...
              <div className="relative overflow-hidden aspect-[16/9]">
                ...
                {dairy.category && (
                  <span className="absolute top-4 left-4 bg-blue-600 text-white text-xs px-3 py-1 rounded-full">
                    <Tag className="inline w-3 h-3 mr-1" />{dairy.category}
                  </span>
                )}
              </div>
            )}
            ...
            <div className="p-5 flex-1 flex flex-col">
              ...
              <Link to={`/dairy/${dairy._id}`} className="group mb-2">
